package peter.screens
{
	...
	public class InitScreen extends screen
	{
		...
		private var input2:TextField = new TextField();
		private var input3:TextField = new TextField();
		...
		private function drawHeader(spr:Sprite):void
		{
			...
			insTf.text = 'Welcome to Treasure Digger by Peter Trice!\nYour objective is to locate the treasures. ' +
				'Click on a square to begin your hunt. Using your honed Treasure Digger instincts, the arrows on the right ' +
				'will point to the location of each treasure.\nBut beware of the monsters randomly located around the treasure field. ' +
				'Find one, and your game is over.\n!!!  Look for warnings that a monster is in an adjacent square !!!';
		...
		{
			var sampleController:TreasureGameController = new TreasureGameController(uint(input1.text),uint(input1.text),uint(input2.text),uint(input3.text));
			this.parent.addChild(sampleController);
